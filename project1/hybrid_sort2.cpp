...
std::vector<int> hybrid_merge2(std::vector<int> &v1, std::vector<int> &v2){
...
void hybrid_merge_sort2(std::vector<int> &v, const double H){
    int n = v.size();
    ...
    if (n > H){
        ...
        hybrid_merge_sort2(v1, H);
        hybrid_merge_sort2(v2, H);
        v = hybrid_merge2(v1, v2);
    } else
        insertion_sort(v);
}

void hybrid_sort2(std::vector<int> &v){
    const int H = pow(v.size(), 1 / 3);
    hybrid_merge_sort2(v, H);
}
